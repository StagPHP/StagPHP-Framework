...
function enqueue_css_backend($data_array){
    ...
    if(!isset($data_array['name']) || !isset($data_array['version'])) return;
    ...
    if(isset($data_array['relative_url'])){
      $resource_dir = get_assets_dir_uri(NULL, TRUE);
      $asset_uri = $resource_dir.$data_array['relative_url'];
    } else if(isset($data_array['absolute_url'])){
      $asset_uri = $data_array['absolute_url'];
    } 
...
function stag_get_enqueued_css_backend(){
    ...
    switch($value['priority']){
        case 'normal':
            ...
            $stylesheet = '<link rel="stylesheet" id="'.$key.'" href="'.$value['asset_uri'].'?ver='.$value['version'].'" type="text/css" media="all"/>';
            /** Pushing array */
        ...
        case 'low':
            ...
            /** Creating Stylesheet HTML link tag */
            $stylesheet = '<link rel="stylesheet" id="'.$key.'" href="'.$value['asset_uri'].'?ver='.$value['version'].'" type="text/css" media="none" onload="'.$js_function.'"/>'."\r\n";
            /** Creating Stylesheet HTML link tag
             * inside noscript tag */
            $stylesheet .= '<noscript><link rel="stylesheet" id="'.$key.'" href="'.$value['asset_uri'].'?ver='.$value['version'].'" type="text/css" media="all"/></noscript>';
            /** Pushing array */
